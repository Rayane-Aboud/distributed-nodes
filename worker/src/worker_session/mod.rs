use tokio::{net::{TcpStream, tcp::{OwnedReadHalf, OwnedWriteHalf}}, sync::mpsc::{self, Sender}, task::JoinHandle};
use common::{deserialize, protocol::{NodeToNode, PeerInfoMessage, WireMessage, WorkerEvent}};
use tokio::io::{BufReader, AsyncWriteExt};

use crate::{utils::read_wire_message, worker_node_info::PeerRegistry};
...
impl PeerSession {
    pub async fn run_outbound(peer: PeerInfoMessage) {
        ...
        let (tx, _writer_handle) = Self::spawn_writer(write_half);
        ...
        Self::peer_reader_loop(tx.clone(),read_half).await;
    }
    ...
    pub async fn run_inbound(
        ...
        match msg {
            WireMessage::NodeToNode(NodeToNode::PeerHello { .. }) => {
                ...
                let (_tx, _writer_handle) = Self::spawn_writer_to_peer(write_half);
                Self::peer_reader_loop(tx_core.clone(),reader.into_inner()).await;
            }
            _ => {
                // Protocol violation
                return;
